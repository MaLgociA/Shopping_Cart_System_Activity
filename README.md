...
**• MANAGE CART**
- This feature allows the user to review and modify the items they’ve selected before checkout. It provides options to REMOVE items from the cart, UPDATE quantities if user decides to change the quantities of the products the user wants to buy, BACK from the main menu, or even CLEAR the entire cart, while automatically adjusting product stock based on those changes. This feature ensures the cart accurately reflects the user’s intended purchase before finalizing the transaction.
